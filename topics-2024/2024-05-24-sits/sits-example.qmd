...
*Update* At first I used tiles, but later went to a small region of interest (roi). When I went from tiles to roi in the `sits_regularize()` the last date had no data and that caused errors.

## Get the data

I am going to get a year of data for a small region.
...
sits_cube_copy(s2_cube,
  output_dir = here::here("tempdir/chp4"),
  roi = roi
)
```
...
reg_cube <- sits_apply(reg_cube,
   ...
   output_dir = here::here("tempdir/chp5"),
   multicores = 4
   )
```
